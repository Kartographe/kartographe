...
def list_migration_column_comments(
    ...
    rows = manager.list_for_entity(
        account, EntityType.DATABASE_MIGRATION_COLUMN, column.id
    )
...
def create_migration_column_comment(
    ...
    comment = manager.create(
        account,
        user,
        entity_type=EntityType.DATABASE_MIGRATION_COLUMN,
        entity_id=column.id,
        value=form.value,
    )
...
@router.post(
    "/bulk",
    operation_id="api_databases_migrations_columns_comments_bulk_create",
    summary="Post several comments on a migration column at once",
    description=(
        "Post 1 to 50 comments on a migration column step in a single call — prefer this over "
        "calling `api_databases_migrations_columns_comments_create` in a loop. Best-effort: each "
        "comment is posted independently, so one failing item does not roll back the others. "
        "Always returns 207; read each `results[].status` and the `created` / `failed` counts. "
        "Any member may post."
    ),
...
def bulk_create_migration_column_comments(
