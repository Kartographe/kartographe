...
@router.post(
    "/bulk",
    operation_id="api_databases_versions_tables_comments_bulk_create",
    summary="Post several comments on a table at once",
    description=(
        "Post 1 to 50 comments on a database table in a single call — prefer this over calling "
        "`api_databases_versions_tables_comments_create` in a loop. Best-effort: each comment is "
        "posted independently, so one failing item does not roll back the others. Always returns "
...
def bulk_create_table_comments(
