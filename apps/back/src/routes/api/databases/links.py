...
List and attach references on a database. Any account member may read and
...
router = APIRouter(
    prefix="/accounts/{account_id}/databases/{database_id}/links", tags=["api.databases.links"]
)
...
@router.get(
    "",
    operation_id="api_databases_links_list",
    summary="List database references",
    description=(
        "List the references attached to a database, oldest first. A reference pointing back at "
...
@router.post(
    "",
    operation_id="api_databases_links_create",
    summary="Attach a reference to a database",
    description=(
        "Attach a reference (a ticket, a document, a design, another Kartographe entity, …) to "
        "a database. Only the URL is required; call `api_links_prefill` first to propose a "
...
@router.post(
    "/bulk",
    operation_id="api_databases_links_bulk_create",
    summary="Attach several references to a database at once",
    description=(
        "Attach 1 to 50 references to a database in a single call — prefer this over calling "
        "`api_databases_links_create` in a loop. Best-effort: each reference is created "
...
def bulk_create_database_links(
