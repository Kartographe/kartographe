...
@router.delete(
    "",
    operation_id="api_databases_complexities_delete",
    summary="Withdraw your estimate on a database",
    description=(
        "Withdraw your complexity estimate on a database. Estimating `null` says \"I cannot "
...
def delete_database_complexity(
    account: CurrentAccountDep,
    member: CurrentAccountUserDep,
    database: CurrentDatabaseDep,
    manager: ComplexityManagerDep,
    ...
    manager.remove(
        account, member, entity_type=EntityType.DATABASE, entity_id=database.id
    )
