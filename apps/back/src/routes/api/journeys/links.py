...
List and attach references on a journey. Any account member may read and
...
router = APIRouter(
    prefix="/accounts/{account_id}/journeys/{journey_id}/links", tags=["api.journeys.links"]
)
...
@router.get(
    "",
    operation_id="api_journeys_links_list",
    summary="List journey references",
    description=(
        "List the references attached to a journey, oldest first. A reference pointing back at "
...
@router.post(
    "",
    operation_id="api_journeys_links_create",
    summary="Attach a reference to a journey",
    description=(
        "Attach a reference (a ticket, a document, a design, another Kartographe entity, …) to "
        "a journey. Only the URL is required; call `api_links_prefill` first to propose a "
...
def create_journey_link(
    ...
    link = manager.create(
        account,
        user,
        entity_type=EntityType.JOURNEY,
...
@router.post(
    "/bulk",
    operation_id="api_journeys_links_bulk_create",
    summary="Attach several references to a journey at once",
    description=(
        "Attach 1 to 50 references to a journey in a single call — prefer this over calling "
        "`api_journeys_links_create` in a loop. Best-effort: each reference is created "
