...
List and give complexity estimates on a scenario step. Any account member may read and estimate.
"""
...
router = APIRouter(prefix="/accounts/{account_id}/journeys/{journey_id}/scenarios/{scenario_id}/steps/{step_id}/complexities", tags=["api.journeys.scenarios.steps.complexities"])
...
@router.get(
    "",
    operation_id="api_journeys_scenarios_steps_complexities_list",
    summary="List step complexity estimates",
...
@router.post(
    "",
    operation_id="api_journeys_scenarios_steps_complexities_create",
    summary="Estimate the complexity of a step",
    description=(
        "Give or update your complexity estimate on a scenario step. Any member may estimate; "
...
def create_step_complexity(
    ...
    complexity = manager.upsert(
        account, member, entity_type=EntityType.JOURNEY_SCENARIO_STEP, entity_id=step.id, value=form.value
    )
...
@router.delete(
    "",
    operation_id="api_journeys_scenarios_steps_complexities_delete",
    summary="Withdraw your estimate on a step",
    description=(
        "Withdraw your complexity estimate on a step. Estimating `null` says \"I cannot "
...
def delete_step_complexity(
    ...
    manager.remove(
        account, member, entity_type=EntityType.JOURNEY_SCENARIO_STEP, entity_id=step.id
    )
