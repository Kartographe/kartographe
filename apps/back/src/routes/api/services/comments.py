...
List and post comments on a service. Any account member may read and post.
"""
...
router = APIRouter(
    prefix="/accounts/{account_id}/services/{service_id}/comments",
    tags=["api.services.comments"],
)
...
@router.get(
    "",
    operation_id="api_services_comments_list",
...
def create_service_comment(
    ...
    comment = manager.create(
        account, user, entity_type=EntityType.SERVICE, entity_id=service.id, value=form.value
    )
...
@router.post(
    "/bulk",
    operation_id="api_services_comments_bulk_create",
    summary="Post several comments on a service at once",
    description=(
        "Post 1 to 50 comments on a service in a single call — prefer this over calling "
        "`api_services_comments_create` in a loop. Best-effort: each comment is posted "
...
def bulk_create_service_comments(
